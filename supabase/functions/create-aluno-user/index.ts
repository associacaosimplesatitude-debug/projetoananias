...
serve(async (req) => {
  ...
  try {
    ...
    if (authError || !user) {
      console.error('Auth error in create-aluno-user:', authError);
      throw new Error('Unauthorized');
    }
    ...
    console.log(`Creating aluno user for email: ${email}, churchId: ${churchId}`);
    ...
    if (createError) {
      ...
      console.log(`Created new aluno user with id: ${userId}`);
    }
    ...
    console.error('Error in create-aluno-user:', error);
