...
import { RefreshCw, Search, Package, CalendarIcon } from "lucide-react";
...
export default function PedidosAmazon() {
  ...
  const { data: pedidos, isLoading } = useQuery({
    queryKey: ["marketplace-pedidos-amazon"],
    queryFn: async () => {
  ...
  const syncMutation = useMutation({
    mutationFn: async () => {
      const { data, error } = await supabase.functions.invoke("bling-sync-marketplace-orders", {
        body: { marketplace: "AMAZON" },
      });
      if (error) throw error;
      return data;
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["marketplace-pedidos-amazon"] });
      toast.success(`Sincronizado! ${data.syncedCount || 0} pedidos da Amazon.`);
    },
    onError: (error: any) => {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Pedidos sincronizados do marketplace Amazon via Bling</p>
        </div>
