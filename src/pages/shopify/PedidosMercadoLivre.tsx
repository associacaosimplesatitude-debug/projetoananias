...
import { RefreshCw, Search, ShoppingCart, CalendarIcon, Upload } from "lucide-react";
...
export default function PedidosMercadoLivre() {
  ...
  const { data: pedidos, isLoading } = useQuery({
    queryKey: ["marketplace-pedidos-mercadolivre"],
    queryFn: async () => {
  ...
  const syncMutation = useMutation({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["marketplace-pedidos-mercadolivre"] });
      toast.success(`Sincronizado! ${data.syncedCount || 0} pedidos do Mercado Livre.`);
    },
    onError: (error: any) => {
  ...
  const importMutation = useMutation({
    mutationFn: async (csvContent: string) => {
      const { data, error } = await supabase.functions.invoke("import-mercadolivre-csv", {
        body: { csvContent },
      });
      if (error) throw error;
      return data;
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Pedidos sincronizados do Mercado Livre via Bling</p>
        </div>
