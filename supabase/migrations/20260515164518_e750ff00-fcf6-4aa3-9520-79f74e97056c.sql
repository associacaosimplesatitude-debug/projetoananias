...
BEGIN
  ...
  mkt_shopee AS (
    ...
    WHERE marketplace = 'SHOPEE'
      AND status_pagamento IN ('paid','Pago')
      AND (order_date AT TIME ZONE 'America/Sao_Paulo')::date = data_ref
  ),
  mkt_ml AS (
    SELECT id, valor_total
  ...
  totais_ontem AS (
    SELECT
      ...
      COALESCE((
        SELECT SUM(valor_total) FROM vendas_balcao
        WHERE status = 'finalizada'
          AND (created_at AT TIME ZONE 'America/Sao_Paulo')::date = v_data_ontem
      ),0) +
      COALESCE((
      ...
      (
        ...
        (SELECT COUNT(*) FROM ebd_shopify_pedidos_mercadopago
          WHERE (status = 'PAGO' OR payment_status IN ('approved','PAGO','paid'))
            AND (created_at AT TIME ZONE 'America/Sao_Paulo')::date = v_data_ontem) +
        (SELECT COUNT(*) FROM vendedor_propostas
          WHERE status IN ('FATURADO','APROVADA_FATURAMENTO','PAGO')
            AND (COALESCE(confirmado_em, updated_at) AT TIME ZONE 'America/Sao_Paulo')::date = v_data_ontem) +
  ...
  -- itens normalizados para mix e produto destaque
  itens_norm AS (
  ...
  -- vendedores top 5 (canais com vendedor_id)
  vend_agg AS (
