...
BEGIN
  -- Permite chamadas autenticadas como admin OU chamadas via service_role (link público)
  ...
  WITH
  ecommerce AS (
    SELECT id, vendedor_id, valor_total, 'shopify'::text AS origem
    ...
    UNION ALL
    SELECT id, vendedor_id, valor_total, 'loja_cg'::text
  ...
  ec_ontem AS (
    SELECT id, valor_total FROM ebd_shopify_pedidos
    WHERE status_pagamento = 'paid'
      AND (order_number IS NULL OR order_number NOT ILIKE 'BLING-%')
      AND (created_at AT TIME ZONE 'America/Sao_Paulo')::date = v_data_ontem
    UNION ALL
    SELECT id, valor_total FROM ebd_loja_pedidos_cg
    WHERE status_pagamento = 'paid'
      AND (created_at AT TIME ZONE 'America/Sao_Paulo')::date = v_data_ontem
  ),
  totais_ontem AS (
    SELECT
      COALESCE((SELECT SUM(valor_total) FROM ec_ontem),0) +
  ...
  -- Itens vindos de jsonb (propostas faturadas, MP, balcão)
  itens_norm AS (
    ...
    -- Itens do E-commerce (Shopify) e Faturados Shopify via tabela de line items
    UNION ALL
    SELECT it.product_title AS titulo,
           COALESCE(it.quantity, 1)::numeric AS quantidade,
           (COALESCE(it.price,0) * COALESCE(it.quantity,1))::numeric AS valor
    FROM ebd_shopify_pedidos_itens it
    WHERE it.pedido_id IN (SELECT id FROM ecommerce WHERE origem='shopify')
       OR it.pedido_id IN (SELECT id FROM faturados_shopify)
  ),
  -- Quantidade extra para canais sem detalhe de itens (loja_cg, marketplaces): 1 por pedido
  itens_fallback AS (
    SELECT (SELECT COUNT(*) FROM ecommerce WHERE origem='loja_cg') +
           (SELECT COUNT(*) FROM mkt_shopee) +
           (SELECT COUNT(*) FROM mkt_ml) AS qtd
  ),
  itens_classif AS (
    SELECT titulo, quantidade, valor,
      CASE
  ...
  mix AS (
    SELECT categoria, SUM(quantidade)::numeric AS quantidade, SUM(valor)::numeric AS valor
    FROM itens_classif GROUP BY categoria
  ),
  destaque AS (
    SELECT titulo, SUM(quantidade)::numeric AS quantidade
    FROM itens_norm
    WHERE titulo IS NOT NULL AND titulo <> ''
    GROUP BY titulo ORDER BY SUM(quantidade) DESC NULLS LAST LIMIT 1
  ),
  ...
  vend_agg AS (
    ...
    ) t GROUP BY vendedor_id
  ),
  vend_top5 AS (
    SELECT v.vendedor_id, vv.nome, vv.foto_url, v.valor, v.pedidos
    FROM vend_agg v LEFT JOIN vendedores vv ON vv.id = v.vendedor_id
    ORDER BY v.valor DESC NULLS LAST LIMIT 5
  ),
  ...
  SELECT jsonb_build_object(
    'totais', jsonb_build_object(
      ...
      'ticket_medio', CASE WHEN (SELECT pedidos FROM totais_hoje) > 0
                           ...
                           ELSE 0 END,
      'produtos_vendidos', COALESCE((SELECT SUM(quantidade) FROM itens_norm), 0) + COALESCE((SELECT qtd FROM itens_fallback), 0),
...
-- Função pública (sem autenticação) que apenas chama a versão admin via service_role implícito
CREATE OR REPLACE FUNCTION public.get_resumo_diario_publico(data_ref date)
RETURNS jsonb
LANGUAGE plpgsql
STABLE SECURITY DEFINER
SET search_path TO 'public'
...
DECLARE
  ...
  v_data_ontem date := data_ref - INTERVAL '1 day';
BEGIN
  -- Sem checagem de role: pensado para ser exposto via link público (rota /admin/resumo-diario?d=...)
  -- Restringe a últimos 90 dias para evitar abuso
  IF data_ref < (CURRENT_DATE - INTERVAL '90 days') OR data_ref > CURRENT_DATE THEN
    RAISE EXCEPTION 'data_fora_intervalo';
  END IF;
...
-- Permite chamar a versão pública sem autenticação
GRANT EXECUTE ON FUNCTION public.get_resumo_diario_publico(date) TO anon, authenticated;
