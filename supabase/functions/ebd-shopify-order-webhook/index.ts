  ...
  try {
    ...
    try {
      ...
      for (const item of lineItems) {
        ...
        // === Criar licença sempre que houver identificador (telefone ou email) ===
        const identificadorWhatsapp = temWhatsapp ? whatsappLimpo : emailComprador;

        {
          ...
          const { error: upsertError } = await supabase
            ...
            .upsert({
              ...
              whatsapp: identificadorWhatsapp,
          ...
          if (upsertError) {
            ...
            if (temWhatsapp) {
              ...
              console.log(`✅ Revista digital license created for ${nomeComprador} (email: ${emailComprador}), revista: ${tituloRevista} — telefone internacional/inválido, usando email como identificador`);
            }
          }

          // === WhatsApp boas-vindas via Meta API direta (template) — APENAS se tiver telefone válido ===
          if (temWhatsapp) {
            try {
            ...
            console.log(`ℹ️ Pedido ${order.order_number}: telefone internacional/inválido, WhatsApp não enviado. Licença criada com email como identificador.`);
          }
        }
