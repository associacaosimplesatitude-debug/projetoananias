...
BEGIN
  ...
  itens_norm AS (
    ...
    FROM mp_link, LATERAL jsonb_array_elements(items_arr) it
    UNION ALL
    SELECT
      (it->>'title') AS title,
      (it->>'sku')   AS sku,
      COALESCE((it->>'quantity')::numeric,0) AS quantity,
      COALESCE((it->>'price')::numeric,0)    AS price
    FROM fat_direto, LATERAL jsonb_array_elements(itens_arr) it
  ),
  ...
  mix AS (
    SELECT categoria, SUM(quantity) AS qtd, SUM(quantity * price) AS valor
    FROM itens_classif GROUP BY categoria
  ),

  destaque AS (
    SELECT title, SUM(quantity) AS qtd
    FROM itens_classif WHERE title IS NOT NULL
    GROUP BY title ORDER BY qtd DESC LIMIT 1
  ),
  ...
  vend_agg AS (
    ...
    GROUP BY vendedor_id ORDER BY valor DESC LIMIT 5
  ),

  vend_top5 AS (
    SELECT v.id AS vendedor_id, v.nome, v.foto_url,
           ROUND(va.valor::numeric, 2) AS valor, va.pedidos
    FROM vend_agg va JOIN vendedores v ON v.id = va.vendedor_id
