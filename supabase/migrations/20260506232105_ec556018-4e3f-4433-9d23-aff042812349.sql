  ...
  SELECT json_build_object(
    ...
    'igrejas_total', (
      ...
      FROM (
        ...
        WHERE status = 'PAGO' AND created_at >= v_start AND created_at < v_end
      ) combined
    ),
    ...
    'advecs', (
      ...
      FROM (
        ...
        WHERE marketplace = 'ADVECS' AND order_date >= v_start AND order_date < v_end
        UNION ALL
        SELECT COALESCE(SUM(esp.valor_total), 0) as total, COUNT(*) as cnt
        FROM ebd_shopify_pedidos esp LEFT JOIN ebd_clientes ec ON esp.cliente_id = ec.id
        WHERE esp.status_pagamento IN ('Pago', 'paid', 'Faturado')
    ...
    'amazon', (
      SELECT json_build_object('valor', COALESCE(SUM(valor_total), 0), 'qtd', COUNT(*))
      FROM bling_marketplace_pedidos
      WHERE marketplace = 'AMAZON' AND order_date >= v_start AND order_date < v_end
    ),

    'shopee', (
      SELECT json_build_object('valor', COALESCE(SUM(valor_total), 0), 'qtd', COUNT(*))
      FROM bling_marketplace_pedidos
      WHERE marketplace = 'SHOPEE' AND order_date >= v_start AND order_date < v_end
    ),

    'mercado_livre', (
      SELECT json_build_object('valor', COALESCE(SUM(valor_total), 0), 'qtd', COUNT(*))
      FROM bling_marketplace_pedidos
      WHERE marketplace = 'MERCADO_LIVRE' AND order_date >= v_start AND order_date < v_end
    ),

    'atacado', (
      SELECT json_build_object('valor', COALESCE(SUM(valor_total), 0), 'qtd', COUNT(*))
      FROM bling_marketplace_pedidos
      WHERE marketplace = 'ATACADO' AND order_date >= v_start AND order_date < v_end
    ),

    'propostas_advecs', (
      SELECT json_build_object('valor', COALESCE(SUM(vp.valor_total - COALESCE(vp.valor_frete, 0)), 0), 'qtd', COUNT(*))
      FROM vendedor_propostas vp LEFT JOIN ebd_clientes ec ON vp.cliente_id = ec.id
      WHERE vp.status IN ('FATURADO', 'PAGO')
        AND vp.created_at >= v_start AND vp.created_at < v_end
    ...
    'pdv_balcao', (
      SELECT json_build_object('valor', COALESCE(SUM(valor_total), 0), 'qtd', COUNT(*))
      FROM vendas_balcao
      WHERE status = 'finalizada' AND created_at >= v_start AND created_at < v_end
    ),
    ...
    'outros_b2b', (
      ...
      FROM (
        ...
        WHERE vp.status IN ('FATURADO', 'PAGO')
          AND vp.created_at >= v_start AND vp.created_at < v_end
          AND ec.tipo_cliente IS NULL
        UNION ALL
        SELECT COALESCE(SUM(esp.valor_total), 0) as total, COUNT(*) as cnt
        FROM ebd_shopify_pedidos esp LEFT JOIN ebd_clientes ec ON esp.cliente_id = ec.id
        WHERE esp.status_pagamento IN ('Pago', 'Faturado')
          AND esp.created_at >= v_start AND esp.created_at < v_end
          AND (esp.order_number IS NULL OR esp.order_number ILIKE 'BLING-%')
          AND ec.tipo_cliente IS NULL
        UNION ALL
        SELECT COALESCE(SUM(mp.valor_total), 0) as total, COUNT(*) as cnt
        FROM ebd_shopify_pedidos_mercadopago mp LEFT JOIN ebd_clientes ec ON mp.cliente_id = ec.id
