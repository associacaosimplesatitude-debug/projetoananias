...
BEGIN
  ...
  com_dias AS (
    ...
    WHERE data_ultima_compra > '-infinity'::timestamptz
       OR ultimo_resultado IN ('interessado','falar_com_consultor','recusou')
  ),
  fechados_mes AS (
    SELECT cliente_id,
    ...
    'kanban_clientes', (
      SELECT COALESCE(json_agg(json_build_object(
        ...
        'coluna_kanban', CASE
          ...
          WHEN cd.ultimo_resultado = 'recusou' THEN 'recusou'
          ELSE 'a_contatar' END
      ...
      WHERE cd.dias_sem_compra > 60
         ...
         OR cd.ultimo_resultado IN ('interessado','falar_com_consultor','recusou')
    )
  ) INTO result;
  RETURN result;
END;
